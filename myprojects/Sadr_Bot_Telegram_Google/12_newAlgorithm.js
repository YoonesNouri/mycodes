...
var webAppUrl = "https://script.google.com/macros/s/AKfycbyVNpY4gvXMwkc8l-cceKWp_zJqSE-42t_TJTN4Hr4QCFYC7PBeekqyFuChqA7r3yVu8g/exec";
  ...
  for (var i = 0; i < data.length; i++) {
    if (data[i][1] == from) {
      ...
      if (currentDate < expirationDate) {
        return true; // User found and contract not expired
      } else {
        sendMessage(from, "Your contract has expired. Please renew your contract.");
        return false; // User found but contract expired
      }
    }
  }
  sendMessage(from, "Your ID is not available. Please register first.");
...
function doPost(e) {
  try {
    ...
    sendMessage(from, "Please enter your name, surname, and phone number in this pattern: name:surname:09127654321");

    var text = contents.message.text;
    var Uinfo = text.split(":");
    storeUserInfo(from, Uinfo[0], Uinfo[1], Uinfo[2]);
    sendMessage(from, "Please wait...");

    if (authenticateUser(from)) {
      // User is authenticated, allowed to store guest information
      sendMessage(from, "Please enter your guest national code and phone number in this pattern: 1234567890:09127654321");

      var GinfoText = contents.message.text;
      var Ginfo = GinfoText.split(":");
      storeGuestInfo(from, Ginfo[0], Ginfo[1]);
      sendMessage(from, "Guest information stored successfully!");
    }
  } catch (error) {
