// شروع 4 دی 1402 و تحویل 8 بهمن 1402 بود
...
var webAppUrl = "https://script.google.com/macros/s/AKfycby7ysSI1LSt_hRo7I5k7Li0NFoSVLlkgysTVN9dRJjHyQOpdU3V_gJ18uSKYpOgpGXOhw/exec";
...
function doPost(e) {
  ...
  try {
    ...
    if (state === States.Unregistered) {
      ...
      sendMessage(userId, "قرارداد شما در تاریخ " + formattedPersianDate + " منقضی شده است. لطفاً نسبت به تمدید آن اقدام کنید. سپس برای وارد کردن اطلاعات مهمان تان /mehman را بزنید.");
      ...
      if (isValidGuestMobile(messageText)) {
        ...
        sendMessage(userId, "لطفا تاریخ حضور مهمان تان را طبق قالب 30/12/1402 (روز ماه سال) وارد کنید.");
      ...
      if (isValidDate(messageText)) {
        ...
        var persianDate = Converter.gregorianToPersian(userExpirationDate.getFullYear(), userExpirationDate.getMonth() + 1, userExpirationDate.getDate());
        var formattedPersianDate = persianDate[2] + '/' + persianDate[1] + '/' + persianDate[0];
        sendMessage(userId, "مطمئن شوید تاریخ حضور مهمان تان برای گذشته یا بعد از اتمام قراردادتان یعنی " + formattedPersianDate + " نباشد و یا قالبش بصورت 30/12/1402 (روز ماه سال) باشد.");
      }
