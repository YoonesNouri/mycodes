...
const handleCompleteUserData = () => {
  const sheetData = {name:"یونس",fanily:"asdqwe",phone:null}
  if (!sheetData.name) {
    sendMessage(userId,"نام را وارد کنید")
  } else if (!sheetData.family) {
     sendMessage(userId,"نام را وارد کنید")
  } else if (!sheetData.phone) {
     sendMessage(userId,"نام را وارد کنید")
  }
